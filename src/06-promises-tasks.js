...
function willYouMarryMe(isPositiveAnswer) {
  return new Promise((resolve, reject) => {
    if (isPositiveAnswer !== undefined && (typeof isPositiveAnswer === 'boolean' || isPositiveAnswer instanceof Boolean)) {
      if (isPositiveAnswer) {
...
function chainPromises(array, action) {
  const asyncAction = async (a, b) => {
    const paramB = await b;
    if (!a) return paramB;
    return action(paramB, a);
  };

  if (array.length === 1) {
    return Promise.resolve(array.pop()).catch(() => Promise.resolve(null));
  }
  return Promise.resolve(array.pop())
    .catch(() => Promise.resolve(null))
    .then((response) => asyncAction(response, chainPromises(array, action)));
}
